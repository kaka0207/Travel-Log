...
import type { AddressData, SearchResult } from "@/modules/mapbox/lib/openstreetmap";
import { Skeleton } from "@/components/ui/skeleton";
import { formatGPSCoordinates } from "@/lib/utils";
import { useGetAddress } from "@/modules/mapbox/hooks/use-get-address";
import { searchOpenStreetMap } from "@/modules/mapbox/lib/openstreetmap";
...
export function ThirdStep({
  ...
  const handleSearch = useCallback(async () => {
    ...
    setIsSearching(true);
    try {
      const results = await searchOpenStreetMap(searchQuery);
  ...
  return (
    <Form {...form}>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <FormItem>
          <FormLabel>搜索地点</FormLabel>
          {/* Search Input */}
          <div className="relative z-20 isolate mb-2">
            <ButtonGroup>
              <Input
                placeholder="搜索地点..."
                value={searchQuery}
            ...
            {searchQuery.trim() && searchResults.length > 0 && (
              <Card className="absolute inset-x-0 top-full z-[1100] mt-1 w-full bg-background p-0 shadow-2xl">
          ...
          <FormControl>
            <div className="relative z-0 h-[400px] w-full overflow-hidden rounded-md border">
              <MapboxComponent
                draggableMarker
