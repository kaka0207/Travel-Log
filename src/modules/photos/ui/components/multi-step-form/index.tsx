...
export default function MultiStepForm({
  ...
  const handleNext = (data: Partial<PhotoFormData>) => {
    ...
    if (step < STEP_CONFIG.length - 1) {
      ...
      const addressProperties = address?.features?.[0]?.properties;
      const context = addressProperties?.context;
      ...
      const cityName =
        countryCode === "JP" || countryCode === "TW"
          ? regionName || placeName || localityName
          : placeName || regionName || localityName;
      ...
      const finalData = {
        ...
        showCameraParameters: updatedData.showCameraParameters ?? true,
        // Keep the complete reverse-geocoded address for display and editing.
        country: context?.country?.name,
        countryCode,
        region: regionName,
        city: cityName,
        district: localityName,
        fullAddress: addressProperties?.full_address || addressProperties?.place_formatted,
        placeFormatted: addressProperties?.place_formatted || addressProperties?.full_address,
      };
      ...
      createPhoto.mutate(finalData, {
        onSuccess: async () => {
          ...
          await queryClient.invalidateQueries(
            trpc.home.getManyLikePhotos.queryOptions({ limit: 12 })
          );
          await queryClient.invalidateQueries(
