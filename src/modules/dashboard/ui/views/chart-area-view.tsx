...
export function ChartAreaView() {
  ...
  const [timeRange, setTimeRange] = React.useState(() =>
    typeof window !== "undefined" && window.innerWidth < 768 ? "1y" : "3y"
  );
  ...
  const peakMonth = React.useMemo(() => {
    ...
    return {
      month: new Date(peak.month + "-01").toLocaleDateString("zh-CN", {
        month: "long",
        year: "numeric",
      }),
      count: peak.count,
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        <CardTitle>照片活动</CardTitle>
        <CardDescription>
          <span className="hidden @[540px]/card:block">
            过去 {years} 年拍摄 {totalPhotos.toLocaleString()} 张照片
            {peakMonth && (
              <span className="ml-2 text-muted-foreground">
                · 峰值：{peakMonth.month}（{peakMonth.count} 张）
              </span>
            )}
          </span>
          <span className="@[540px]/card:hidden">
            {totalPhotos.toLocaleString()} 张 · 近 {years} 年
          </span>
        </CardDescription>
        <CardAction>
          <ToggleGroup
          ...
          >
            ...
            <ToggleGroupItem value="3y">3 年</ToggleGroupItem>
            <ToggleGroupItem value="5y">5 年</ToggleGroupItem>
          </ToggleGroup>
